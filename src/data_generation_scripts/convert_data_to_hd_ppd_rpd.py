...
def get_all_data_in_df(data_path: Path, num_of_users: int):
    ...
    for file_name in tqdm(next(walk(data_path))[2]):
        ...
        try:
            df = pd.read_csv(
                file_path, delimiter="\t", quoting=csv.QUOTE_NONE, encoding="ISO-8859-1"
            )  # , usecols = ['PARTICIPANT_ID','TEST_SECTION_ID','PRESS_TIME', 'RELEASE_TIME', 'KEYCODE']) #delimiter='\t', quotechar="")
...
def to_horizontal_state(df: pd.DataFrame, num_of_strokes: int):
    ...
    for user_id in tqdm(np.unique(df["PARTICIPANT_ID"])):
        ...
        for idx in range(
            0, len(list(user_df["PRESS_TIME"])) - num_of_strokes + 1, num_of_strokes
        ):
...
def generate_ppd_rpd_hd(data_df: pd.DataFrame, num_of_strokes: int):
    ...
    for i in range(1, num_of_strokes):
        data_df["PPD-" + str(i)] = (
            data_df["press-" + str(i)] - data_df["press-" + str(i - 1)]
        )
        data_df["RPD-" + str(i)] = (
            data_df["release-" + str(i)] - data_df["press-" + str(i - 1)]
        )

    for i in range(num_of_strokes):
        data_df["HD-" + str(i)] = (
            data_df["release-" + str(i)] - data_df["press-" + str(i)]
        )
    return data_df
...
def clean_data(data_df: pd.DataFrame, num_of_strokes: int):
    data_df.drop(
        columns=["press-" + str(i) for i in range(num_of_strokes)], inplace=True
    )
    data_df.drop(
        columns=["release-" + str(i) for i in range(num_of_strokes)], inplace=True
    )
...
if __name__ == "__main__":
    ...
    argParser.add_argument(
        ...
        "--data_path",
        type=Path,
        default=Path("/dataset/Keystrokes/files"),
        help="dataset path",
    )
    argParser.add_argument(
        "-sp",
        "--save_path",
        type=Path,
        default=Path("/dataset/full_data.csv"),
        help="dataset path",
    )
    argParser.add_argument(
        "-ns",
        ...
        type=int,
        default=200,
        help="num of strokes to use in split",
    )
    argParser.add_argument(
        "-nu",
        "--num_of_users",
